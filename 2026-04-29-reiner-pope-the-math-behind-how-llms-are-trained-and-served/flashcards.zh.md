...
### Q2. $t_{\mathrm{compute}}$ 的公式是什么？

$$t_{\mathrm{compute}} = \frac{B \cdot N_{\mathrm{active}}}{\mathrm{FLOPs}}$$

其中 $B$ 是 batch size，$N_{\mathrm{active}}$ 是 active parameters，FLOPs 是硬件的计算吞吐。

### Q3. $t_{\mathrm{mem}}$ 的公式是什么？提示：它既包含权重读取，也包含 KV cache 读取。
...
### Q4. 在脑中画出横轴为 batch size、纵轴为 latency 的图：画出 $t_{\mathrm{compute}}$、KV fetch 和 weight fetch 的曲线，然后把给定 batch size 下对应总延迟的曲线加粗。
...
因此 $B \geq 300 / \mathrm{sparsity}$。这里的 $\mathrm{sparsity}$ 按 active / total parameters 计算；如果用 total / active 表示稀疏度倍率，则是 $300 \times$ 稀疏度倍率。
...
$C_{\mathrm{pretrain}} = 6 \times N_{\mathrm{active}} \times D_{\mathrm{pretrain}}$，即 $6ND$ 公式，包含 forward + backward。

$C_{\mathrm{RL}} = (2 \,\mathrm{to}\, 6) \times N_{\mathrm{active}} \times D_{\mathrm{RL}} \times \mathrm{inefficiency}$。如果不在 rollout 上训练、只做 forward，则系数为 2；如果训练 rollout，最高到 6；inefficiency 来自 decode 阶段较低的 MFU。

$C_{\mathrm{inference}} = 2 \times N_{\mathrm{active}} \times D_{\mathrm{inference}} \times \mathrm{inefficiency}$。这里只做 forward pass；decode 阶段 MFU 较低。

### Q3. 为什么你可能会天真地预期 $C_{\mathrm{pretrain}} = C_{\mathrm{RL}} = C_{\mathrm{inference}}$？

如果预训练、RL 和推理成本之间存在权衡，比如更多预训练可以让同等质量所需的 RL 或推理更少，反之亦然，那么最优点大致会出现在三者相等的位置。

### Q4. 假设 decode 的 MFU 只有 prefill 的 $\tfrac{1}{3}$，求解 $D_{\mathrm{pretrain}} = D_{\mathrm{RL}} = D_{\mathrm{inference}}$ 的关系。
...
### Q6. Chinchilla rule 是 $D_{\mathrm{optimal}} \approx 20 \times N_{\mathrm{active}}$。如果一个前沿模型有 100B active parameters，并在 200T tokens 上预训练，它比 Chinchilla-optimal 多多少？
...
在交叉点处，$t_{\mathrm{compute}} = t_{\text{KV fetch}}$：
...
代入：$N_{\mathrm{active}} \approx 100\text{B}$，$\mathrm{len}_{\mathrm{ctx}} = 200\text{K}$，得到 $\mathrm{bytes/token} \approx 1.7\text{ KB}$。
