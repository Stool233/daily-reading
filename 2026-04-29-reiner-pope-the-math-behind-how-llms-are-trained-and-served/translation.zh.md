...
- $B$ 是 batch size。
- $N_{\text{active}}$ 是每个 token 实际激活的参数数。
- FLOPs 是硬件计算吞吐。
...
- $N_{\text{total}}$ 是模型总参数数。
- $\text{len}_{\text{ctx}}$ 是上下文长度。
- $\text{KV}_{\text{bytes/token}}$ 是每个 token 的 KV cache 字节数。
- $\text{mem\_bw}$ 是内存带宽。
...
$$B = \frac{\text{FLOPs}}{\text{mem\_bw}} \cdot \frac{N_{\text{total}}}{N_{\text{active}}}$$

现代 GPU 上，按 FP4 乘法和字节数换算后，$\text{FLOPs}/\text{mem\_bw}$ 常见量级约为 300。因此：

$$B \approx 300 \cdot \frac{N_{\text{total}}}{N_{\text{active}}}$$

如果把 $\frac{N_{\text{total}}}{N_{\text{active}}}$ 理解为稀疏度倍率，那么 MoE 越稀疏，所需 batch size 越大。例如 DeepSeek V3 每次激活 32/256 个 expert，也就是 8 倍稀疏度，粗略需要：
...
$$C_{\text{pretrain}} = 6 \times N_{\text{active}} \times D_{\text{pretrain}}$$
...
$$C_{\text{RL}} = (2 \text{ to } 6) \times N_{\text{active}} \times D_{\text{RL}} \times \text{inefficiency}$$

$$C_{\text{inference}} = 2 \times N_{\text{active}} \times D_{\text{inference}} \times \text{inefficiency}$$
...
$$D_{\text{optimal}} \approx 20 \times N_{\text{active}}$$
...
\frac{B \cdot \text{len}_{\text{ctx}} \cdot \text{bytes/token}}{\text{mem\_bw}}$$

消去 $B$，可得：

$$\text{bytes/token} =
\frac{\text{mem\_bw}}{\text{FLOPs}} \cdot
\frac{N_{\text{active}}}{\text{len}_{\text{ctx}}}$$

如果 $\text{FLOPs}/\text{mem\_bw} \approx 300$，$N_{\text{active}} \approx 100\text{B}$，交叉点 $\text{len}_{\text{ctx}} = 200\text{K}$，则：

$$\text{bytes/token} \approx \frac{1}{300} \cdot \frac{100\text{B}}{200\text{K}} \approx 1.7\text{KB}$$
